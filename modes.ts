...
3.  **FUNCTION CALL:** Call the 'set_timecodes' function **only once** with a single array containing all the generated timecode objects. All timecodes MUST be in the format 'HH:MM:SS' (e.g., 00:00:05).
...
3.  **FUNCTION CALL:** Call the 'set_timecodes' function **only once** with a single array of all the transcription objects. All timecodes MUST be in the format 'HH:MM:SS' (e.g., 00:02:10).
...
set_timecodes with the timecode of the sentence in the video. The timecodes MUST be in the format 'HH:MM:SS'.`,
  },
...
object sent to set_timecodes with the timecode of the bullet point in the video. The timecodes MUST be in the format 'HH:MM:SS'.`,
    isList: true,
  },
...
3.  **FUNCTION CALL:** Call the 'set_timecodes' function **only once** with a single array containing all the suggested edits as timecode objects. All timecodes MUST be in the format 'HH:MM:SS'. For example, an entry might have a time of "00:00:21" and text of "Filler word: 'um'. Suggest cutting."`,
    isList: true,
  },
...
4.  **FUNCTION CALL:** Call the 'set_timecodes' function **only once** with a single array containing all the B-roll suggestions as timecode objects. All timecodes MUST be in the format 'HH:MM:SS'. For example, an entry might have a time of "00:01:15" and text of "B-roll suggestion: Close-up of hands typing on a keyboard."`,
    isList: true,
  },
...
4.  **FUNCTION CALL:** Call the 'set_timecodes' function **only once** with a single array containing all the sound cue suggestions as timecode objects. All timecodes MUST be in the format 'HH:MM:SS'. For example, an entry might have a time of "00:00:45" and text of "Sound Cue: Add a 'whoosh' sound effect as the car passes."`,
    isList: true,
  },
  Table: {
    emoji: '🗓️',
...
3.  **FUNCTION CALL:** Call the 'set_timecodes_with_objects' function **only once** with a single array containing all the identified objects and their corresponding timestamps and descriptions. All timecodes MUST be in the format 'HH:MM:SS'.
